...
def FFT(y,t):
    ...
    T=L*dt #1/T=Fs/L
    freq=arange(0,Fs/2,1/T) #list frequencies up to Nyquist frequency
...
def CEPSTRUM(y,t):
    ...
    C=abs(ifft(log(abs(fft(y))**2)))**2
    NumUniquePts=int(ceil((L+1)/2))
    C=C[0:NumUniquePts]
    q=arange(0,NumUniquePts,1)*dt
    return q,C
...
def SIDFT(X,D):
    ...
    for n in range(0,N,1):
        for k in range(0,N,1):
            x[n]=x[n]+exp(-1j*2*pi*k*D/N)*X[k]*exp(1j*2*pi*k*n/N)
    return x/N
