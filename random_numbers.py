...
def exp_sampler(lambd):
    ...
    x = -1.0 / lambd * log(random.random() / lambd)
...
def exp_hist(lambd, nruns=10000, nbins=100):
    result = linspace(1, nruns, nruns)  # empty array of length nruns for results
...
# The histograms for N=5,10,20 are plotted together with the analytics solution
# to the central limit theorem
def expmu_hist(lambd, nruns=10000, nbins=100):
    # number of samples:
    N = array([5, 10, 20])
    # create a vector to hold the results
    result = zeros([len(N), nruns])
    ...
    x = linspace(-result.min(), result.max(), 1000)
    # this allows linspace to scale the values of x according to the lambd value
    mu = lambd ** -1  # analytical mean
    p = zeros([len(N), len(x)])
    for i in range(len(N)):
        sig2 = (N[i] * lambd ** 2) ** -1  # analytical variance which depends on N
        p[i, :] = ((2 * pi * sig2) ** -0.5) * exp(-(x - mu) ** 2 / 2.0 * sig2 ** -1)
...
# random matrices:
def goe_eigs(N):
	A = random.randn(N, N)  # create a real N x N matrix whose elements
...
# random matrices:	
def gue_eigs(N):
	A = random.randn(N, N) + 1j * random.randn(N, N)  # A=U+iV
	B = (2 ** -1) * (A + conj(A.T)) 
	D = linalg.eigvals(B)  # D = eigenvalues vector
	D = real(D)
	# include real() to prevent certain warnings from popping up in ipython
...
def random_matrix_hist(N, fun, nruns=1000, nbins=50):
	b = zeros([nruns, N])  # create an array to hold all the results of 1000 runs
	for i in range(nruns):
	...
	breshape = bnorm.reshape(size(bnorm), 1)  # reshape into column vector

	# To obtain eig spacing:
	DELTA = sort(bnorm)  # sort works row-wise i.e. sorts elements in each row only
	DELTA = diff(DELTA)  # diff works on the elements on each row only
	DELTA = DELTA.reshape(1, size(DELTA))  # reshape into a row vector
	mDELTA = mean(DELTA)  # obtain mean of DELTA i.e. eig spacing
	# I realise mean works faster on row rather than column vectors
	DELTA = DELTA * (mDELTA ** -1)  # divide by the mean value
	DELTA = DELTA.reshape(size(DELTA), 1)  # reshape back to column vector
	...
	x = linspace(-2, 2, 1000)
	pw = 1.0 * (2 * pi) ** -1 * (4 - x ** 2) ** 0.5  # Wigner Semi Circle Law
	plt.plot(x, pw, 'r')
	delta = linspace(0, 3, 1000)
	fw = pi * delta * 0.5 * exp((-pi * delta ** 2) * 0.25)  # Wigner Surmise
