...
def dde2():
    ...
    delayindex = int(tstar / dt) # for array calling
    ...
    for i in range(delayindex):
        y0[0] = y0[0] - 1.0 / delayindex # P history
        y0[1] = y0[1] - 1.0 / delayindex # Q history
        Y[i+1, :] = y0
    ...
    print("Time elapsed: {:.2f}s".format(endtime)) 
    
    plt.figure(figsize = (8,8))
    plt.plot(X[int(len(X)*0.2):, 1], X[int(len(X)*0.2):, 0]) 
    plt.xlabel('Pressure')
    plt.ylabel('Flux')
    plt.grid('on')
    plt.show() 
