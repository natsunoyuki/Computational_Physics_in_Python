...
def transfer_matrix(E, V, L, V0):
    ...
    assert len(V) == len(L)
    N = len(L)
    V = append(V, V0)  # Append V0 to the end of V for usage in the for loop
    ...
    transfer = 0.5 * array([[1.0 + k[-1] / k[0], 1 - k[-1] / k[0]], [1 - k[-1] / k[0], 1 + k[-1] / k[0]]])
    ...
    for n in range(N):
        ...
        transl = array([[exp(1j * k[n] * L[n]), 0], [0, exp(-1j * k[n] * L[n])]])
        # jump from V[n] to V[n+1]:
        jump = 0.5 * array([[1.0 + k[n] / k[n + 1], 1 - k[n] / k[n + 1]], [1 - k[n] / k[n + 1], 1 + k[n] / k[n + 1]]])
        # now calculate the rest of the transfer matrix by "appending"
        # the other matrix multiplications infront of the current
        # transfer matrix:
        transfer = dot(transl, transfer)
        transfer = dot(jump, transfer)
...
def barrier_plot():
    E = linspace(1, 250, 700)  # range of E to be used
    V = array([50])  # various potential steps
    L = array([1])  # potential step lengths
    V0 = 0  # potential outside
    T = zeros(len(E))  # transmission results
    R = zeros(len(E))  # reflection results
...
def resonant_barrier_plot2():
    ...
    Q = find_bound_states(50, 250, array([50]), array([1]), 250)
    print('The bound states energies are: ')
    print Q
...
#T=transfer_matrix(1,array([10]),array([1]),0)
#print T
#barrier_plot()
